...
# Gathering all data
        ...
        datas.to_pickle('datasetX.pkl')
    else:
        datas = pd.read_pickle('datasetX.pkl')
...
def normalizeDatas(datas):
    l_datas = datas.copy()
    l_datas.loc[:,'ErroPitch':'dErroP'] = (l_datas.loc[:,'ErroPitch':'dErroP'] - l_datas.loc[:,'ErroPitch':'dErroP'].min())/(l_datas.loc[:,'ErroPitch':'dErroP'].max()-l_datas.loc[:,'ErroPitch':'dErroP'].min())
    return l_datas
def normalizeNewData(data,datas):    
    ...
    toClass['ErroPitch'].loc[(toClass['ErroPitch'] > 1)] = 1
    toClass['ErroPitch'].loc[(toClass['ErroPitch'] < 0)] = 0
    toClass['dErroP'].loc[(toClass['dErroP'] > 1)] = 1
    toClass['dErroP'].loc[(toClass['dErroP'] < 0)] = 0
...
def generateFuzzySys(datas):
    ...
    ## Regras dos extremos
    #R1 = ctrl.Rule(Error['NL'],ElevStick['SM'])
    #allRules.append(R1)
    #R2 = ctrl.Rule(Error['PL'],ElevStick['DM'])
    #allRules.append(R2)
    #R3 = ctrl.Rule(((Error['NM'] | Error['NS']) & (dError['NL'] | dError['NM'])) | (Error['Z'] & dError['NL']) | (Error['NM'] & dError['NS']),ElevStick['SM'])
    #allRules.append(R3)
    #R4 = ctrl.Rule(((Error['PM'] | Error['PS']) & (dError['PL'] | dError['PM'])) | (Error['Z'] & dError['PL']) | (Error['PM'] & dError['PS']),ElevStick['DM'])
    #allRules.append(R4)
    #R5 = ctrl.Rule((Error['PS'] & dError['NL']) | (Error['Z'] & dError['NM']) | (Error['NS'] & dError['NS']) | (Error['NM'] & dError['Z']),ElevStick['SS'])
    #allRules.append(R5)
    ...
    #allRules.append(R8)
    #R9 = ctrl.Rule((Error['PM'] & dError['Z']) | (Error['PS'] & dError['PS']) | (Error['Z'] & dError['PM']) | (Error['NS'] & dError['PL']),ElevStick['DS'])
    #allRules.append(R9)
    ...
    #allRules.append(R2)
    #R3 = ctrl.Rule(
    #    (Error['NL'] & dError['NL']), ElevStick['SL']
    #    )
    #allRules.append(R3)
    #R4 = ctrl.Rule(
    #    (Error['Z']) | 
    ...
    #allRules.append(R4)
    #R5 = ctrl.Rule(
    #    (Error['PL'] & dError['PL']) | (Error['PM'] & dError['PL'] ) | (Error['PL'] & dError['PM'] ) | (Error['PL'] & dError['PS'] ) | (Error['PM'] & dError['PM'] ), ElevStick['DL']
    #    )
    #allRules.append(R5)
    #R6 = ctrl.Rule(
